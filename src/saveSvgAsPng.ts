...
export const prepareSvg = (el, options, done) => {
  requireDomNode(el);
  const {
    left = 0,
    top = 0,
...
export const svgAsDataUri = (el, options, done) => {
  requireDomNode(el);
...
export const svgAsPngUri = (el, options, done) => {
  requireDomNode(el);
  const {
...
export const download = (name, uri, options) => {
...
export const saveSvg = (el, name, options) => {
...
export const saveSvgAsPng = (el, name, options) => {
  const downloadOpts = downloadOptions(); // don't inline, can't be async
  return requireDomNodePromise(el)
