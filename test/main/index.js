document.getElementById('file').addEventListener('change', function handleFileSelect(evt) {
    ...
    if (file) {
        ...
        reader.onload = e => {
            ...
            exportSvg
                .svgAsPngUri($el.find('.load-target svg')[0], null)
...
function inlineTest(title, $el, saveOptions, testOptions) {
    ...
    exportSvg
        ...
        .then(({ uri, width, height }) => row.find('.preview').html(
            '<div>' +
            '<img src="' + uri + '" />' +
            '<div>width=' + width + ', height=' + height + '</div>' +
            '</div>'
...
$sandbox.find('.render').click(() => {
    ...
    try {
        ...
        $sandbox.find('.save').unbind('click').click(() => exportSvg.
